...
def push_anomaly_to_fritz(oid: str, survey: str, group_id: int, token: str) -> bool:
    ...
    payload = {"group_ids": [int(group_id)]}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"  [STEP 1 ERROR] Failed to push {oid}. Code: {response.status_code}"
            )
            return False
...
def push_annotation_to_fritz(
    oid: str, verdict: dict, group_id: int, token: str
) -> bool:
    """
    ...
    try:
        ...
        if response.status_code in [200, 400]:
            ...
            print(
                f"  [STEP 2 ERROR] Failed Annotation for {oid}. Code: {response.status_code}"
            )
            return False
    except requests.exceptions.RequestException as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Push CYOA anomalies & LLM annotations to SkyPortal UI"
    )
    ...
    parser.add_argument(
        "--group-id", required=True, help="Fritz Group ID to push objects to"
    )
    parser.add_argument(
        "--delay", type=float, default=1.5, help="Delay between API calls"
    )
    ...
    for idx, verdict in enumerate(verdicts):
        ...
        print(
            f"[{idx + 1}/{len(verdicts)}] Processing {oid} ({verdict.get('primary_class', 'Unknown')})..."
        )
