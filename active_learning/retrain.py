...
def extract_4_source_labels(annotations, feature_vector, mock_label=0.0):
    ...
    for ann in annotations:
        ...
        # Source 4: LLM AI Review (Weight 0.7 scaled by confidence)
            ...
            if (
                "interesting" in llm_verdict.lower()
                or "candidate" in llm_verdict.lower()
            ):
                training_data.append(
                    (feature_vector, 1.0, 0.7 * conf_multiplier, "llm")
                )
            elif "noise" in llm_verdict.lower() or "artifact" in llm_verdict.lower():
                training_data.append(
                    (feature_vector, 0.0, 0.7 * conf_multiplier, "llm")
                )
...
def train_active_learning_model(group_id, all_features, all_labels, all_weights):
    ...
    if len(np.unique(all_labels)) < 2:
        print(
            "[WARNING] The model cannot train. It requires both positive (1) and negative (0) feedback."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="CYOA Active Learning Retrain Cron Job"
    )
    ...
    parser.add_argument(
        "--local-cache",
        required=True,
        help="Path to local triage jsonl with object IDs",
    )
    ...
    print(
        f"Scanning {len(known_objects)} registered anomalies for SkyPortal feedback..."
    )

    print("\nReading locally cached Delta feature vectors...")
    try:
        parquet_features = pd.read_parquet(
            "d:/KayDoesResearch/boom-fitting-ml/pipeline/features.parquet"
        )
        ...
        numeric_features = numeric_features.drop(
            columns=[c for c in numeric_features.columns if "label" in c],
            errors="ignore",
        )

        print(
            f"Successfully loaded {numeric_features.shape[1]}-dim mathematical features for {len(numeric_features)} total objects."
        )
    except Exception as e:
    ...
    for idx, obj in enumerate(known_objects):
        ...
        print(
            f"  [{idx + 1}/{len(known_objects)}] Digging SkyPortal annotations for {oid}..."
        )
        ...
        extracted_points = extract_4_source_labels(
            annotations, feature_vector, mock_label=float(idx % 2)
        )
