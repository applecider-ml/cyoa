...
from model import LightCurveCompressor  # noqa: E402
...
def main():
    ...
    parser.add_argument(
        "--model-path", required=True, help="Path to best_model.pt from RTF training"
    )
    parser.add_argument(
        "--output-dir",
        required=True,
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device("cpu")
    print(f"\n{'=' * 60}")
    print("RTF Mass Inference Sweep — Phase 5.2")
    ...
    print(
        f"  Percentile threshold ({args.anomaly_percentile:.0f}th): {threshold_val:.4f}"
    )
    ...
    with open(csv_path, "w") as f:
        ...
        for rank, idx in enumerate(flagged_idx, 1):
            f.write(
                f"{rank},{oids_arr[idx]},{combined[idx]:.6f},"
